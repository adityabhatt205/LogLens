...
""",
    ...
    typer.echo("  Custom rules: loglens rules validate my_rule.yml")
    typer.echo("  List rules  : loglens rules list")
    _pause(no_pause)

    # ── 4. Error Tracking ──────────────────────────────────────────────────
    _h("4 / 7  -- Error Tracking  (--track-errors)")
    typer.echo("\n  Errors are deduplicated by fingerprint and persisted to SQLite.\n")
    typer.echo("  $ loglens errors list\n")
    ...
    typer.echo("\n  Extras: loglens errors show <fp>  |  loglens errors new --since 24h")
    typer.echo("          loglens errors regression  (errors that reappeared after silence)")
    _pause(no_pause)
    ...
    typer.echo("  $ loglens findings list\n")
    ...
    typer.echo("  $ loglens scan app.log --detect-anomalies\n")
    ...
    typer.echo("\n  Training:  loglens anomaly learn app.log")
    typer.echo("  Status:    loglens anomaly status")
    typer.echo("  Reset:     loglens anomaly reset --source app.log")
    _pause(no_pause)
  ...
  Quick-start:
    ...
    loglens scan /var/log/syslog --track-errors --detect-anomalies
    loglens errors list
    loglens findings list
    loglens llm info
    loglens llm explain <fingerprint>
    loglens llm ask "what happened last night?"
  ...
  Run this demo again:  loglens demo run
  Run without pauses:   loglens demo run --no-pause
""")
...
def _load_cfg(config: Path | None):
    """Load config, falling back to the LOGLENS_CONFIG env var if set."""
    import os

    from loglens.config import Config

    if config is None:
        env_path = os.environ.get("LOGLENS_CONFIG", "")
        if env_path:
...
def demo_seed(
    ...
    with 'loglens demo clear' without affecting real data.
    """
    import json
    import sqlite3

    from loglens.storage.errors_schema import ERRORS_SCHEMA_SQL
    from loglens.storage.findings_schema import FINDINGS_SCHEMA_SQL
    from loglens.storage.schema import SCHEMA_SQL
    ...
    if findings_inserted == 0 and errors_inserted == 0:
        typer.echo(
            typer.style(
                "  Demo data already present. Run 'loglens demo clear' first to reseed.",
    ...
    typer.echo("\n  Open the dashboard:  loglens serve")
    typer.echo("  Remove demo data:    loglens demo clear")
    typer.echo(f"  {sep}\n")
...
def demo_clear(
    ...
    Only records tagged by 'loglens demo seed' are deleted.
