...
Two tiers:
  ...
  2. ChromaDB vector search — optional, activated when ``chromadb`` is
     installed and an index has been built via ``loglens llm index``.
...
def _chroma_search(
    ...
    try:
        col = client.get_collection("loglens")
    except Exception:
        return []
...
def build_chroma_index(db_path: Path, chroma_path: Path, client) -> int:
    ...
    try:
        chroma.delete_collection("loglens")
    except Exception:
        pass
    col = chroma.create_collection("loglens")
